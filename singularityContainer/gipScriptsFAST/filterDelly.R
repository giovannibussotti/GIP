...
#set --minDV to the minimum amount of reads supporting the SV in the test sample (e.g. 10) and --maxDVreference to the max amount of reads supporting the SV in the reference sample (set it to zero if you wanna select just the very clean SV).
#you can also filter by minFractionDVDRtest and maxFractionDVDRreference, but the fraction does not tell you about the absolute support of the SV, so for instance a SV supported just by one read in the test sample, by zero reads in the reference sample would still be accepted (unless minDV is also used)
...
#Example: Rscript filterDelly.R --useENDfield yes --vcfFile DEL.vcf --SVTYPE DEL --test hamster_immunoCompetent_H154p2spleen --reference hamster_immunoCompromised_cycloSpleen --minPE 50 --minPercentDVDR 20 --maxPercentDVDRreference 1 --PRECISE no --outName hamster_immunoCompetent_H154p2spleen__hamster_immunoCompromised_cycloSpleen__SV --minDV 10 --maxDVreference 0 --minFractionDVDRtest 0.1 --minMAPQ 50  --maxFractionDVDRreference 0 
#################################################
#		CONFIGURATION
#################################################
...
parser$add_argument("--minPE" , type="integer" , help="min paired-end support of the structural variant summing up all conditions [default %(default)s]" , default="0")
parser$add_argument("--minDV" , type="integer" , help="min paired-end support of the structural variant in the test sample [default %(default)s]" , default="0")
parser$add_argument("--maxDVreference" ,  help="max paired-end support of the structural variant in the reference sample [default %(default)s]" , default="Inf")
parser$add_argument("--minPercentDVDR",type="double", help="min (DV/(DV+DR))*100 in the test sample (i.e. min percent of reads supporting the structural variant in the test sample)  [default %(default)s]" , default="0")
parser$add_argument("--maxPercentDVDRreference", help="max (DV/(DV+DR))*100 in the reference sample (i.e. max percent of reads supporting the structural variant in the reference sample)  [default %(default)s]" , default="Inf")
...
if (maxPercentDVDRreference == "Inf") { maxPercentDVDRreference <- Inf }
if (maxDVreference == "Inf") { maxDVreference <- Inf }
maxDVreference <- as.integer(maxDVreference)
maxPercentDVDRreference <- as.numeric(maxPercentDVDRreference)
...
if(!is.na(badSequencesBed) && length(rowRanges(vcf)) > 0 ){  
	...
	vcf <- vcf[allPercOvBad <= maxBanSeq,]
}
...
goodDVDRselector <- geno(vcf)[["DV"]][,test] >= minDV & geno(vcf)[["DV"]][,reference] <= maxDVreference &  geno(vcf)[["DVDRpercent"]][,test] >= minPercentDVDR & geno(vcf)[["DVDRpercent"]][,reference] <= maxPercentDVDRreference
