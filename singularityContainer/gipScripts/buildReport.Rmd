...
medianGenomeCov   <- read.table(paste0(sample,".karyotype.medianCoverage"))$V1
...
The genome was divided in contiguous bins spanning `r binSize` nucleotides. For each bin GIP:
...
The median of the genomic bin intervals is **`r medianGenomeCov`**.
  
### Somy score normalization  
To calculate the somy score, the mean sequencing coverage of each genomic bin with mean MAPQ score > `r minMAPQbin` was normalized by the median of all bins and multiplied by two.  
**Figure `r figureX`** and **Figure `r figureX + 1`** are two alternative somy score distribution visualizations. 
  
  
```{r karyotypeBoxplot  , out.width = "50%" , fig.cap=paste0("Figure ",figureX -1,": Boxplot somy score distribution")}
knitr::include_graphics(paste0(sample,".karyotype.boxplot.png"))
figureX <- figureX + 1
```
  
  
```{r karyotypeRidges  , out.width = "50%" , fig.cap=paste0("Figure ",figureX -1,": Density plot, somy score distributio")}
knitr::include_graphics(paste0(sample,".karyotype.ridges.png"))
figureX <- figureX + 1
```
...
```{r SNVplots0  , out.width = "100%" , fig.cap=paste0("Figure ",figureX -1,": SNV density per chromosome")}
knitr::include_graphics(paste0(sample,"_freebayesFiltered/single_totVarPerChr.png"))
figureX <- figureX +1
```
```{r SNVplots1  , out.width = "100%" , fig.cap=paste0("Figure ",figureX -1,": VRF distribution")}
knitr::include_graphics(paste0(sample,"_freebayesFiltered/single_allDensities.png"))
figureX <- figureX +1
```
```{r SNVplots2  , out.width = "100%" , fig.cap=paste0("Figure ",figureX -1,": SNV position vs VRF")}
knitr::include_graphics(paste0(sample,"_freebayesFiltered/single_VRFvsPosFaceting.png"))
figureX <- figureX +1
```
```{r SNVplots3  , out.width = "100%" , fig.cap=paste0("Figure ",figureX -1,": SNV depth vs VRF")}
...
Downstream quality filters were applied on the delly's output to reduce the number of false positives and ease graphical representation.  
The filtered output is available from [here](`r paste0("../samples/",sample,"/",sample,"_dellyFiltered/output.vcf.gz")`).  

```{r dellyStats ,  results='asis'}
invDf <- tryCatch(read.table(paste0(sample,"_dellyFiltered/INV.bed"),stringsAsFactors=F,header=F,sep="\t") , error=function(cond){} )
delDf <- tryCatch(read.table(paste0(sample,"_dellyFiltered/DEL.bed"),stringsAsFactors=F,header=F,sep="\t") , error=function(cond){} )
insDf <- tryCatch(read.table(paste0(sample,"_dellyFiltered/INS.bed"),stringsAsFactors=F,header=F,sep="\t") , error=function(cond){} )
bndDf <- tryCatch(read.table(paste0(sample,"_dellyFiltered/BND.bed"),stringsAsFactors=F,header=F,sep="\t") , error=function(cond){} )
dupDf <- tryCatch(read.table(paste0(sample,"_dellyFiltered/DUP.bed"),stringsAsFactors=F,header=F,sep="\t") , error=function(cond){} )
```
...
In total the SV filtered output includes: 

* `r length(bndDf[,1])` break end translocations (links): [BND coordinates](`r paste0("../samples/",sample,"/",sample,"_dellyFiltered/BND.bed")`) 
* `r length(insDf[,1])` insertions (red): [INS coordinates](`r paste0("../samples/",sample,"/",sample,"_dellyFiltered/INS.bed")`) 
* `r length(dupDf[,1])` duplications (orange): [DUP coordinates](`r paste0("../samples/",sample,"/",sample,"_dellyFiltered/DUP.bed")`) 
* `r length(delDf[,1])` deletions (blue): [DEL coordinates](`r paste0("../samples/",sample,"/",sample,"_dellyFiltered/DEL.bed")`) 
* `r length(invDf[,1])` inversions (green): [INV coordinates](`r paste0("../samples/",sample,"/",sample,"_dellyFiltered/INV.bed")`) 
